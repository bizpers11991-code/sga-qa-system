// api/_lib/JobSheetPrintView.tsx
import React from 'react';
import { Job, DailyJobSheetData } from '../../types';
import SgaLogo from './SgaLogo';
        ...
        React.createElement('table', { className: "header-table" },
            React.createElement('tbody', null,
                React.createElement('tr', null,
                    ...
                    React.createElement('td', { className: "header-value" }, job.jobNo)
                ),
                React.createElement('tr', null,
...
const JobSheetPrintView: React.FC<{ job: Job }> = ({ job }) => {
    ...
    return (
        React.createElement('div', { className: "report-container" },
            ...
            jobSheetData.jobSheetImages && jobSheetData.jobSheetImages.length > 0 && (
                React.createElement(React.Fragment, null,
                    React.createElement('h3', { className: "avoid-break" }, 'Attachments'),
                    React.createElement('div', { style: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' } },
                        jobSheetData.jobSheetImages.map((img, i) => (
                            React.createElement('div', { key: i }, React.createElement('img', { src: img.data, alt: img.name, style: { width: '100%', border: '1px solid #ccc' } }))
                        ))
                    )
                )
            )
